...
from .envs import WebAgentTextEnv
from .utils import *
from .engine.goal import get_reward
...
class WebEnv:
    ...
    def get_valid_buttons(self):
        btntext_to_btnnumber, btnnumber_to_btntext, has_search_bar  = self.env.get_available_buttons()
        
        if has_search_bar:  # only search action available
            atts = self.session['goal']['attributes']
            query = self.session['goal']['query']
            inst = self.session['goal']['instruction_text']
            texts = self.get_search_texts(atts, query, inst)
            ...
            valids = []  # and text.startswith('b')]
            for btn_text, btn_number in btntext_to_btnnumber.items():
                # ban buy when options not completed
                if btn_text == 'buy now' and self.ban_buy:
                    cur_options = len(self.session['options'])
                    all_options = len(
                        self.env.server.product_item_dict[self.session["asin"]]['customization_options'])
                    if cur_options != all_options:
                        continue
                valids.append(f'click[{btn_number}]')
            ...
            if self.reduce_click and len(valids) > 20:
                ...
                trivial_buttons = []
                for valid in valids:
                    if btnnumber_to_btntext[valid[6:-1]] in ["back to search", "< prev", "next >", "description", "features", "reviews", "buy now"]:
                        important_buttons.append(valid)
                    else:
                        trivial_buttons.append(valid)
                valids = important_buttons + random.sample(trivial_buttons, 16 - len(important_buttons))
    ...
    def step(self, action):
        ...
        # convert button_version to real actions
        # not efficient appraoch
        if self.button_version and action.startswith('click['):
            _, btnnumber_to_btntext, _ = self.env.get_available_buttons()
            btntext = btnnumber_to_btntext[action[6:-1]]
            if self.click_item_name and btntext in self.asin2name:
                btntext = 'item - ' + self.asin2name[btntext]
            action = f'click[{btntext}]'
        ...
        info.update({'valid': self.get_valid_actions() if not self.button_version else self.get_valid_buttons(), 
