...
class WebAgentTextEnv(gym.Env):
    ...
    def __init__(
        ...
        self.button_version = self.kwargs.get('button_version', 0)
        self.reset()
    ...
    def get_available_buttons(self):
        """Returns list of available buttons at the current step"""
        html_obj = self._parse_html()
        
        texts = html_obj.findAll(text=True)
        filtered_texts = filter(tag_buttons, texts)
        
        search_bar = html_obj.find(id='search_input')
        has_search_bar = True if search_bar is not None else False
        
        buttontext_to_buttonnumber = {}
        buttonnumber_to_buttontext = {}
        counter = 1
        for text in filtered_texts:
            if text == '\n': continue
            buttontext_to_buttonnumber[f"{text}".lower()] = f"button {counter}"
            buttonnumber_to_buttontext[f"button {counter}"] = text.lower()
            counter +=1
        return buttontext_to_buttonnumber, buttonnumber_to_buttontext, has_search_bar
    ...
    def convert_html_to_text(self, html, simple=False):
        ...
        if simple:
            ...
            for t in visible_texts:
                if t == '\n': continue 
                if t.parent.parent.get('id') == 'instruction-text': continue # added by Moghis
                if t.parent.name == 'button':  # button
                    processed_t = f'[button {counter}] {t} [button_]' if self.button_version else f'[button] {t} [button_]'
                    counter += 1
                elif t.parent.name == 'label':  # options
                    if f'"{t}"' in self.state['url']:
                        processed_t = f'  [clicked button {counter}] {t} [clicked button_]' if self.button_version else f'  [clicked button] {t} [clicked button_]'
                    ...
                    if f'{t}' in self.server.user_sessions[self.session]['asins']:
                        processed_t = f'\n[clicked button {counter}] {t} [clicked button_]' if self.button_version else f'\n[clicked button] {t} [clicked button_]'
                        counter += 1
                    else:
                        processed_t = f'\n[button {counter}] {t} [button_]' if self.button_version else f'\n[button] {t} [button_]'
                        counter += 1
...
def tag_buttons(element):
        include = {'button', 'label'}
        return (
            (element.parent.name in include and not isinstance(element, Comment)) or
            (element.parent.get('class') == ["product-link"] and not isinstance(element, Comment))
        )
